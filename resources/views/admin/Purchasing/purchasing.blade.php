...
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-10">
        <h2>Data Purchasing</h2>
        <ol class="breadcrumb">
            <li class="breadcrumb-item">
                <a href="{{url('/')}}">Home</a>
            </li>
            <li class="breadcrumb-item active">
                <strong>Purchasing Order List </strong>
            </li>
        </ol>
    </div>
    <div class="col-lg-2">
...
function Hxr(data){
    ...
    $.ajax({
        type: 'POST',
        url:"{{route('List Purchasing')}}",
        data: formData,
        processData: false,
        contentType: false,	
        success: function(e , data , response) {
          // console.log(data)
            alert(e.message);
...
$(document).on("dblclick", "#dataTableBuilder>tbody>tr", function (e) {
    let inv=btoa($(this).find('.sorting_1')[0].innerHTML);
    window.location.href = '{{route('Detail Purchasing')}}?code='+inv;
    console.log(inv);
});
