...
<script>
    function showmodalprivilege() {
        $.get("{{route('Erp Supplier')}}", function (e) {
    ...
    function changesupplier(e) {
        let id = e.id;
        supplierpost = $("#" + id).data('id');

        $("#suppliertext").text('');
        $("#suppliertext").text($("#" + id).data('text'));
    ...
    function Hxr(data) {
        ...
        formData.append("flag", supplierpost);
        $.ajaxSetup({
            headers: {
    ...
    function chekcountUsPrivilage() {
        ...
        $('.cheksupplierlist').each(function () {
            if ($(this).is(":checked")) {
                chekoption++;
            }
        });
