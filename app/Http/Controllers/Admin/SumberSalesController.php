...
class SumberSalesController extends Controller
{
  ...
  public function index(Request $request){
    if($request->ajax()){
    $url    = '/api/GetDataSumberSales';
    $method = 'GET';
    $route  = 'List Sumber Sales';
    $data   = NULL ;

    $crud   = new SendToApi();
    $SumberSales = $crud->crud($url , $method , $data);
    $DataTables = [];
    foreach($SumberSales->data as $row){
      ...
      $DataTables[]=array(
        ...
        "Action"           => '<a href="/sumber-sales/edit?code='.$edit.'" class="btn btn-outline btn-link"><i class="fa fa-edit" ></i> Edit</a>
                               <a href="/sumber-sales/delete?code='.$delete.'" class="btn btn-outline btn-link"><i class="fa fa-trash" ></i> Non Active</a>',
      );
    }
      return response()->json([
        "draw"  => intval($request->input('draw')),
        "recordsTotal" => $SumberSales->data,
        "recordsFiltered" => $SumberSales->data,
        "data"  => $DataTables
      ]);
          ...
          ['className'=>'btn btn-primary btn-outline','titleAttr'=>'Tambah ','text'=>'<i class="fa fa-sliders" aria-hidden="true"></i>',
              ...
              $(node).removeClass(\'dt-button\')
                ...
                'action'=>'function ( e, dt, node, config ){
                  window.location.href="/sumber-sales/add";
                }'  
          ],
    ...
    return view('admin.SumberSales.sumbersales' , compact('html'));
  ...
  public function add_sumber_sales(Request $request){
    ...
    if($request -> isMethod('GET')){
      return view('admin.SumberSales.sumbersalesAdd');
    }else{
      $request->validate([
        'nama'        => 'required',
        'code'        => 'required',
  ...
  public function edit_sumber_sales(Request $request){
    ...
    $id   = $exp[1];
    if($request->isMethod('GET')){
      $url    = '/api/find_sumber_sales/'.$id;
      $method = 'GET';
      $data   = NULL ;
      $crud   = new SendToApi();
      $data   = $crud->crud($url , $method,$data);

      return view('admin.SumberSales.sumbersalesEdit' , ['SumberSales'   => $data]);
  ...
  public function delete_sumber_sales(Request $request){
    $code = base64_decode($request->get('code'));
    $exp  = explode('::',$code);
    $id   = $exp[1];
    $data   = [
    ...
    $url    = '/api/delete_sumber_sales/'.$id;
    $method = 'GET';
    $route  = 'List Sumber Sales';
    $crud   = new SendToApi();
    $action = $crud->crud($url , $method , $data);
